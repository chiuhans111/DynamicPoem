...
`)}
  ...
  ${E.mainStart()}
    ...
    `,o="",u=L("x",t.dataType,t.dims.length),d=["rank"],[f,p,m,_,y]=wa(r,s);return f.push(...pe(t.dims,r)),{name:e,shaderCache:{hint:`${n.cacheKey};${m};${_};${y}`,inputDependencies:d},getRunData:()=>({outputs:[{dims:r,dataType:t.dataType}],dispatchGroup:{x:Math.ceil(N.size(r)/64)},programUniforms:f}),getShaderSource:v=>va(v,u,t.dims.length,r.length,s,a,o,t.dataType===10?-65504:-1e5,p,m,_,y)}},w_=(e,t)=>{Cr(e.inputs),e.compute(Sa("MaxPool",e.inputs[0],!1,t))},v_=e=>{let t=e.storage_order,i=e.dilations,n=xa(e);if(t!==0)throw new Error("column major storage order is not yet supported for MaxPool");if(n.ceilMode!==0)throw new Error("using ceil() in shape computation is not yet supported for MaxPool");let s={storageOrder:t,dilations:i,...n,cacheKey:""};return{...s,cacheKey:Tf(s)}},$_=e=>{let t=e.format;return{format:t,...Ta,cacheKey:t}},x_=(e,t)=>{Cr(e.inputs),e.compute(Sa("GlobalMaxPool",e.inputs[0],!0,t))}}),Sf,If,C_,T_,r$=K(()=>{ye(),we(),Fe(),ve(),Sf=(e,t)=>{if(e.length<2||e.length>3)throw new Error("DequantizeLinear requires 2 or 3 inputs.");if(e.length===3&&e[1].dims===e[2].dims)throw new Error("x-scale and x-zero-point must have the same shape.");if(e.length===3&&e[0].dataType!==e[2].dataType)throw new Error("x and x-zero-point must have the same data type.");if(e[0].dataType===6&&e.length>2)throw new Error("In the case of dequantizing int32 there is no zero point.");if(e[1].dims.length!==0&&e[1].dims.length!==1&&e[1].dims.length!==e[0].dims.length)throw new Error("scale input must be a scalar, a 1D tensor, or have the same rank as the input tensor.");if(e.length>2){if(e[0].dataType!==e[2].dataType)throw new Error("x and x-zero-point must have the same data type.");if(e[1].dims.length!==e[2].dims.length)throw new Error("scale and zero-point inputs must have the same rank.");if(!e[1].dims.map((i,n)=>i===e[2].dims[n]).reduce((i,n)=>i&&n,!0))throw new Error("scale and zero-point inputs must have the same shape.")}if(t.blockSize>0){if(e[1].dims.length===0||e[1].dims.length===1&&e[1].dims[0]===1)throw new Error("blockSize must be set only for block quantization.");if(!e[1].dims.map((s,r)=>r===t.axis||s===e[0].dims[r]).reduce((s,r)=>s&&r,!0))throw new Error("For block qunatization, scale input shape to match the input shape except for the axis");if(e[1].dims.length!==e[0].dims.length)throw new Error("For block qunatization the scale input rank must be the same as the x rank.");let i=e[0].dims[t.axis],n=e[1].dims[t.axis];if(t.blockSize<Math.ceil(i/n)||t.blockSize>Math.ceil(i/(n-1)-1))throw new Error("blockSize must be with in the range [ceil(dI / Si), ceil(dI / (Si - 1) - 1)].")}},If=(e,t)=>{let i=N.normalizeAxis(t.axis,e[0].dims.length),n=e[0].dataType,s=n===3,r=e[0].dims,a=e[1].dataType,o=N.size(r),u=n===3||n===2,d=u?[Math.ceil(N.size(e[0].dims)/4)]:e[0].dims,f=e[1].dims,p=e.length>2?e[2]:void 0,m=p?u?[Math.ceil(N.size(p.dims)/4)]:p.dims:void 0,_=f.length===0||f.length===1&&f[0]===1,y=_===!1&&f.length===1,v=We(o),C=_&&(!u||v===4),$=C?v:1,b=C&&!u?v:1,T=L("input",u?12:n,d.length,b),x=L("scale",a,f.length),E=p?L("zero_point",u?12:n,m.length):void 0,z=de("output",a,r.length,$),O=[T,x];E&&O.push(E);let M=[d,f];p&&M.push(m);let R=[{type:12,data:o/$},{type:12,data:i},{type:12,data:t.blockSize},...pe(...M,r)],F=le=>{let ne=[{name:"output_size",type:"u32"},{name:"axis",type:"u32"},{name:"block_size",type:"u32"}];return`
      ...
      }`};return{name:"Range",shaderCache:{hint:`${n}`},getShaderSource:u,getRunData:()=>({outputs:[{dims:r,dataType:n}],dispatchGroup:{x:Math.ceil(a/64)},programUniforms:o})}},S_=e=>{let t=0,i=0,n=0;e.inputs[0].dataType===6?(t=e.inputs[0].getInt32Array()[0],i=e.inputs[1].getInt32Array()[0],n=e.inputs[2].getInt32Array()[0]):e.inputs[0].dataType===1&&(t=e.inputs[0].getFloat32Array()[0],i=e.inputs[1].getFloat32Array()[0],n=e.inputs[2].getFloat32Array()[0]),Ue.webgpu.validateInputContent&&Ef(t,i,n),e.compute(kf(t,i,n,e.inputs[0].dataType),{inputs:[]})}}),Af,zf,I_,E_,s$=K(()=>{ye(),we(),Fe(),ve(),Af=(e,t,i,n)=>{if(e!=="none"&&n!=="i32"&&n!=="u32"&&n!=="f32")throw new Error(`Input ${n} is not supported with reduction ${e}.`);let s=`{
                var oldValue = 0;
                loop {
                  let newValueF32 =`,r=`;
 ...
 * limitations under the License.
 * =============================================================================
 */const b$="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ .,!?'",w$=64,v$=128,$$=256,x$=4,C$=4,T$={chars:b$,seqLen:w$,hiddenSize:v$,intermediateSize:$$,numLayers:x$,numHeads:C$};class S${constructor(){this.session=null,this.config=T$,this.chars=this.config.chars,this.vocabSize=this.chars.length+1,this.seqLen=this.config.seqLen,this.unkId=this.chars.length,this.charToId={},this.idToChar={},[...this.chars].forEach((t,i)=>{this.charToId[t]=i,this.idToChar[i]=t}),this.idToChar[this.unkId]="#"}async init(){try{{const n=import.meta.url,s=n.substring(0,n.lastIndexOf("/")+1);Ue.wasm.wasmPaths=s}const i=(await Qw(async()=>{const{default:n}=await import("./model-PxsmnY7E.js");return{default:n}},[],import.meta.url)).default;console.log("Loading model from:",i),this.session=await Ao.create(i,{executionProviders:["wasm"]})}catch(t){throw console.error("Failed to load model.onnx:",t),new Error(`Could not load ONNX model: ${t.message}`)}return!0}async predict(t){if(!this.session)throw new Error("Poet not initialized");const i=this.seqLen,n=new Int32Array(i).fill(this.unkId);for(let d=0;d<Math.min(t.length,i);d++){const f=t[d];n[d]=this.charToId[f]!==void 0?this.charToId[f]:this.unkId}const s=BigInt64Array.from(n,d=>BigInt(d)),r=new Mt("int64",s,[1,i]),o=(await this.session.run({input_ids:r})).logits.data;let u="";for(let d=0;d<i;d++){const f=d*this.vocabSize;let p=-1/0,m=0;for(let _=0;_<this.vocabSize;_++){const y=o[f+_];y>p&&(p=y,m=_)}u+=this.idToChar[m]||""}return u}}const I$=""+new URL("type-C9Z3ofy3.mp3",import.meta.url).href,E$=(e,t)=>{const i=e.__vccOpts||e;for(const[n,s]of t)i[n]=s;return i},k$={class:"container"},A$=["onMouseover","onTouchstart","data-index"],z$=30,Oi=40,mp=20,Er=1,O$={__name:"App",setup(e){const t=new S$,i=fn(""),n=fn(!1),s=fn("Initializing..."),r=new Audio(I$);r.volume=.4,r.preservesPitch=!1;let a=0,o=0;const u=()=>{const C=Date.now();if(a>0&&C-o>z$){const $=Math.min(1,a*.1),b=Math.random()*.2+.8,T=$*b;r.volume=T,r.playbackRate=.98+Math.random()*.04,r.currentTime=0,r.paused&&r.play().catch(()=>{}),o=C+Math.random()*10,a=0}},d=Oi*mp;let f="#".repeat(d);const p=fn(new Array(d).fill(0)),m=t.seqLen-Er;let _=0;const y=async()=>{try{for(let R=0;R<d;R++){let F=.05;R>0&&(F+=(p.value[R-1]-p.value[R])*.1),R<d-1&&(F+=(p.value[R+1]-p.value[R])*.1),p.value[R]+=F}let C="";for(let R=0;R<d;R++)p.value[R]>1&&(p.value[R]=1),Math.random()>p.value[R]+.7?p.value[R]>0?C+="aeiou"[Math.floor(Math.random()*5)]:C+="#":C+=f[R];let $=0,b=0,T=-1;for(let R=0;R<d;R++)C[R]==="#"&&$++,R>=m&&C[R-m]==="#"&&$--,R>=m-1&&$>T&&(T=$,b=R-m+1);T==0&&(b=_,_+=5,_>d-m&&(_=0),_<0&&(_=d-m));let x=b-Er,E="";for(;x<0;)x++,E+=" ";let z=E+C.slice(x,b+m);for(;z.length<t.seqLen;)z+=" ";const O=(await t.predict(z)).slice(Er,Er+m);b=Math.max(0,Math.min(b,f.length-m));let M="";for(let R=0;R<m;R++){if(z[R+Er]!=="#"&&Math.random()<.5){M+=f[b+R];continue}O[R]!=="#"?(M+=O[R],O[R]!==f[b+R]&&(p.value[b+R]=0,a++)):M+=f[b+R]}u(),f=f.slice(0,b)+M+f.slice(b+m),i.value=f}catch(C){s.value="Inference Error: "+C.message}n.value||setTimeout(y,5)},v=C=>{const $=C.touches[0],b=document.elementFromPoint($.clientX,$.clientY),T=b==null?void 0:b.getAttribute("data-index");T!==null&&(p.value[Number(T)]=-1)};return Xp(async()=>{try{await t.init(),s.value="Ready",y()}catch(C){s.value="Error: "+C.message}}),Qp(()=>{n.value=!0}),(C,$)=>(Ms(),Bs("div",k$,[Mi("div",{class:"poem-grid",onTouchmove:Vw(v,["prevent"])},[(Ms(),Bs(Tt,null,cu(mp,b=>Mi("div",{key:b,class:"row"},[(Ms(),Bs(Tt,null,cu(Oi,T=>Mi("span",{onMouseover:x=>p.value[(b-1)*Oi+(T-1)]=-1,onTouchstart:x=>p.value[(b-1)*Oi+(T-1)]=-1,"data-index":(b-1)*Oi+(T-1),key:T,class:"char",style:Yn({opacity:p.value[(b-1)*Oi+(T-1)]+.1})},Ba(i.value[(b-1)*Oi+(T-1)]),45,A$)),64))])),64))],32),$[0]||($[0]=Mi("br",null,null,-1)),Mi("div",{class:Xn(["status",{error:s.value.startsWith("Error")}])}," Status: "+Ba(s.value),3)]))}},R$=E$(O$,[["__scopeId","data-v-5622b81b"]]);Gw(R$).mount("#app");
